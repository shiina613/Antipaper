...
export default function Home() {
  ...
  async function handleSelectCitations(
    ...
    const requestDocumentId = report.document_id;
    const results = await Promise.allSettled(
      pageNumbers.map((pageNumber) => getDocumentPage(report.document_id, pageNumber)),
    );
    if (
      requestToken !== citationRequestRef.current ||
      requestSessionToken !== sessionTokenRef.current ||
  ...
  async function handleAskQuestion(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (requestSessionToken !== sessionTokenRef.current || requestDocumentId !== documentId) return;
      setMessages((current) => [
        ...current,
        {
          role: "assistant",
      ...
      if (requestSessionToken === sessionTokenRef.current && requestDocumentId === documentId) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      ...
      try {
        ...
        if (cancelled || sessionToken !== sessionTokenRef.current) return;
        ...
        if (!cancelled && sessionToken === sessionTokenRef.current) {
          timeout = window.setTimeout(() => void tick(), 1800);
        }
      }
    }
