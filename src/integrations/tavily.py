...
class TavilySearchClient:
    ...
    def from_env(cls) -> "TavilySearchClient | None":
        ...
        return cls(
            ...
            timeout_seconds=float(os.getenv("TAVILY_TIMEOUT_SECONDS", "4")),
...
class RelatedDocumentFinder:
    ...
    def from_env(cls) -> "RelatedDocumentFinder":
        return cls(
            ...
            max_references=int(os.getenv("RELATED_DOCUMENT_MAX_REFERENCES", "5")),
        )

    def find(self, document: NormalizedDocument, *, deadline_seconds: float = 20.0) -> list[RelatedDocument]:
        """Resolve at most five proven mentions without delaying the main report."""
        mentions = self.extract_mentions(document)[: self.max_references]
        if not mentions:
            return []
        executor = ThreadPoolExecutor(max_workers=min(3, len(mentions)), thread_name_prefix="tavily")
        futures = [executor.submit(self._resolve, mention) for mention in mentions]
        done, _ = wait(futures, timeout=max(0.1, deadline_seconds))
        executor.shutdown(wait=False, cancel_futures=True)
        by_future = {future: index for index, future in enumerate(futures)}
        resolved: list[RelatedDocument] = []
